...
dms=("gdm" "sddm" "lightdm" "none")
select DM in "${dms[@]}"; do
  case "$DM" in
    gdm|sddm|lightdm|none) break ;;
    *) echo "Invalid choice." ;;
  esac
done
echo "-> Selected $DM"
...
# -------------------------------
{
  ...
  case "$WM" in
    i3)
      ...
      echo 'environment.systemPackages = with pkgs; [ alacritty rofi firefox networkmanager dmenu ];'
      ;;
    dwm)
      echo 'services.xserver.enable = true;'
      echo 'services.xserver.windowManager.dwm.enable = true;'
      echo 'environment.systemPackages = with pkgs; [ st dmenu slstatus firefox networkmanager ];'
      ;;
    hyprland)
      echo 'programs.hyprland.enable = true;'
      echo 'environment.systemPackages = with pkgs; [ alacritty waybar rofi firefox networkmanager xdg-desktop-portal-hyprland ];'
      ;;
  esac
